...
restore_local_state() {
  ...
  if [[ -n "${BACKUP_NOTIFIER:-}" && -f "$BACKUP_NOTIFIER" ]]; then
    mv "$BACKUP_NOTIFIER" "$ROOT/config/codex-notifier"
  else
    rm -f "$ROOT/config/codex-notifier"
  fi
}
...
if [[ -f "$ROOT/config/codex-notifier" ]]; then
  BACKUP_NOTIFIER="$TMP_ROOT/codex-notifier.bak"
  cp "$ROOT/config/codex-notifier" "$BACKUP_NOTIFIER"
fi
rm -f "$ROOT/config/codex-notifier"

HOME_DIR="$TMP_ROOT/home/tester"
mkdir -p "$HOME_DIR/.ssh" "$HOME_DIR/projects/secrets" "$HOME_DIR/.codex"
...
(
  ...
  export GOPRIVATE=
  export GONOSUMDB=
  "$ROOT/bin/codex-docker-ctrl" start > "$TMP_ROOT/start.out" 2> "$TMP_ROOT/start.err"
)

if [[ -x "$ROOT/config/codex-notifier" ]]; then
  ok "codex-notifier auto-created from example"
else
  fail "codex-notifier was not auto-created"
fi

CONTEXT_FILES=("$TMPDIR_TEST"/codex-docker-context.*)
if [[ -f "${CONTEXT_FILES[0]}" ]] \
  && grep -Rqs '/AGENTS.md:.*AGENTS.md:ro' "$TMPDIR_TEST"/codex-docker-context.* \
  && grep -Rqs '/CLAUDE.md:.*CLAUDE.md:ro' "$TMPDIR_TEST"/codex-docker-context.*; then
  ok "global AGENTS.md and CLAUDE.md overrides created"
else
  fail "missing AGENTS.md/CLAUDE.md override files"
fi

if ls "$TMPDIR_TEST"/codex-docker-ssh.* >/dev/null 2>&1 \
  && grep -Rqs '.ssh/known_hosts:.*known_hosts:ro' "$TMPDIR_TEST"/codex-docker-ssh.*; then
  ok "known_hosts override created"
else
  fail "missing known_hosts override"
fi

if ls "$TMPDIR_TEST"/codex-docker-excludes.* >/dev/null 2>&1 \
  && grep -Rqs "/dev/null:$HOME_DIR/projects/.env:ro" "$TMPDIR_TEST"/codex-docker-excludes.* \
  && grep -Rqs "$HOME_DIR/projects/secrets:ro,size=0" "$TMPDIR_TEST"/codex-docker-excludes.*; then
...
if grep -q 'COMPOSE_PROJECT_NAME=codex-docker' "$LOG"; then
...
if grep -q "Container 'codex-docker' is running." "$TMP_ROOT/start.out"; then
